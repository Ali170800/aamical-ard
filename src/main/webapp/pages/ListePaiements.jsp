...
<form action="<%= request.getContextPath() %>/supprimerMultiplePaiements" method="post" id="formSuppression">
    ...
    <div style="margin-bottom: 10px;">
        <button type="submit" class="btn btn-supprimer"
                onclick="return confirm('Voulez-vous vraiment supprimer les paiements sélectionnés ?');">
            ...
            <tr>
                ...
                <td>
                    <div class="actions">
                        ...
                        <a href="<%= request.getContextPath() %>/supprimerPaiement?id=${paiement.id}" class="btn btn-supprimer"
                           onclick="return confirm('Supprimer ce paiement ?');">Supprimer</a>
                    </div>
                </td>
            </tr>
        </c:forEach>
...
function filtrerTableau() {
    ...
    for (let i = 0; i < tr.length; i++) {
        ...
        tr[i].style.display =
            (nom.indexOf(filter) > -1 || prenom.indexOf(filter) > -1)
            ? ""
            : "none";
    }
}
    ...
    <form method="post" action="recherchePaiement">
        ...
        <select name="mois">
            ...
            <c:forEach var="i" begin="1" end="12">
                <option value="${i}" ${moisRecherche == i ? 'selected' : ''}>${i}</option>
            </c:forEach>
        </select>
        ...
        <select name="annee">
            ...
            <c:forEach var="i" begin="2023" end="2035">
                <option value="${i}" ${anneeRecherche == i ? 'selected' : ''}>${i}</option>
            </c:forEach>
        </select>
...
<c:if test="${not empty resultatsFiltrage}">
    <div class="section">
        ...
        <table>
            <tr>
                <th>Nom</th><th>Prénom</th><th>Montant</th>
                <th>Mois</th><th>Année</th><th>Statut</th><th>Appartement</th>
            </tr>
