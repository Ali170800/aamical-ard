...
<aside class="w-64 bg-gray-900 text-white flex flex-col h-screen fixed top-0 left-0 shadow-xl z-10">
    ...
    <nav id="sidebarMenu" class="flex-grow py-2 px-4 space-y-1 overflow-y-auto">
        ...
        <a href="<%= request.getContextPath() %>/upload/ajouterPublication.jsp" class="menu-item flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-800 hover:text-indigo-400 text-xs font-semibold transition-all"><i class="fas fa-image text-gray-400 w-4 text-center"></i> Ajouter une publication</a>
...
<main class="flex-grow ml-64 p-10 bg-gray-50 min-h-screen">
    ...
    <div class="flex justify-end items-center mb-8">
        <div class="flex items-center gap-6">
           <a href="<%= request.getContextPath() %>/notifications"
            ...
            <div class="flex items-center gap-3">
                <div class="text-right"><p class="text-sm font-semibold text-gray-800">Administrateur</p><p class="text-xs text-gray-500">Connecté</p></div>
   ...
   <div class="bg-gradient-to-r from-indigo-600 to-indigo-800 text-white rounded-3xl shadow-xl shadow-indigo-600/10 p-10 mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
    ...
    <!-- RACCOURCIS -->
    <h2 class="text-sm font-black text-gray-800 uppercase tracking-wider mb-6">Raccourcis Principaux</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">

        <!-- Élections -->
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 hover-card">
            <div class="flex items-center gap-4 mb-5"><div class="w-12 h-12 rounded-2xl bg-violet-50 text-violet-600 flex items-center justify-center text-base"><i class="fas fa-vote-yea"></i></div><div><h2 class="text-xs font-black text-gray-800">Élections</h2><span class="text-[9px] font-bold text-gray-400 uppercase">Module de vote</span></div></div>
            <a href="<%= request.getContextPath() %>/admin/creer-election" class="block text-center py-2.5 bg-gray-900 hover:bg-indigo-600 text-white text-[10px] font-bold rounded-xl transition-colors">➕ Créer une élection</a>
        </div>

        <!-- Étudiant -->
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 hover-card">
            <div class="flex items-center gap-4 mb-5"><div class="w-12 h-12 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center text-base"><i class="fas fa-user-graduate"></i></div><div><h2 class="text-xs font-black text-gray-800">Étudiant</h2><span class="text-[9px] font-bold text-gray-400 uppercase">Inscription</span></div></div>
            <a href="<%= request.getContextPath() %>/pages/ajoutetu/AjouterEtudiant.jsp" class="block text-center py-2.5 bg-gray-900 hover:bg-indigo-600 text-white text-[10px] font-bold rounded-xl transition-colors">➕ Ajouter étudiant</a>
        </div>

        <!-- Logement -->
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 hover-card">
            <div class="flex items-center gap-4 mb-5"><div class="w-12 h-12 rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center text-base"><i class="fas fa-bed"></i></div><div><h2 class="text-xs font-black text-gray-800">Logement</h2><span class="text-[9px] font-bold text-gray-400 uppercase">Gestion</span></div></div>
            <a href="<%= request.getContextPath() %>/formulaire-logement" class="block text-center py-2.5 bg-gray-900 hover:bg-indigo-600 text-white text-[10px] font-bold rounded-xl transition-colors">➕ Ajouter logement</a>
        </div>

        <!-- Paiement -->
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 hover-card">
            <div class="flex items-center gap-4 mb-5"><div class="w-12 h-12 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center text-base"><i class="fas fa-wallet"></i></div><div><h2 class="text-xs font-black text-gray-800">Paiement</h2><span class="text-[9px] font-bold text-gray-400 uppercase">Comptabilité</span></div></div>
            <a href="<%= request.getContextPath() %>/afficherFormulairePaiement" class="block text-center py-2.5 bg-gray-900 hover:bg-indigo-600 text-white text-[10px] font-bold rounded-xl transition-colors">➕ Ajouter paiement</a>
        </div>

        <!-- Dépenses -->
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 hover-card">
            <div class="flex items-center gap-4 mb-5"><div class="w-12 h-12 rounded-2xl bg-cyan-50 text-cyan-600 flex items-center justify-center text-base"><i class="fas fa-coins"></i></div><div><h2 class="text-xs font-black text-gray-800">Dépenses</h2><span class="text-[9px] font-bold text-gray-400 uppercase">Activités</span></div></div>
            <a href="<%= request.getContextPath() %>/formulaireDepense" class="block text-center py-2.5 bg-gray-900 hover:bg-indigo-600 text-white text-[10px] font-bold rounded-xl transition-colors">➕ Ajouter dépense</a>
        </div>

        <!-- Publication -->
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 hover-card">
            <div class="flex items-center gap-4 mb-5"><div class="w-12 h-12 rounded-2xl bg-pink-50 text-pink-600 flex items-center justify-center text-base"><i class="fas fa-image"></i></div><div><h2 class="text-xs font-black text-gray-800">Publication</h2><span class="text-[9px] font-bold text-gray-400 uppercase">Communauté</span></div></div>
            <a href="<%= request.getContextPath() %>/upload/ajouterPublication.jsp" class="block text-center py-2.5 bg-gray-900 hover:bg-indigo-600 text-white text-[10px] font-bold rounded-xl transition-colors">➕ Ajouter publication</a>
        </div>

        <!-- Voir Publications -->
        <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 hover-card">
            <div class="flex items-center gap-4 mb-5"><div class="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center text-base"><i class="fas fa-newspaper"></i></div><div><h2 class="text-xs font-black text-gray-800">Publications</h2><span class="text-[9px] font-bold text-gray-400 uppercase">Actualités</span></div></div>
            <a href="<%= request.getContextPath() %>/liste-publications" class="block text-center py-2.5 bg-gray-900 hover:bg-indigo-600 text-white text-[10px] font-bold rounded-xl transition-colors">👁 Voir publications</a>
        </div>
...
<script>
    ...
    searchInput.addEventListener('input', function () {
        ...
        menuItems.forEach(item => { item.classList.toggle('hidden-menu', term !== '' && !item.textContent.toLowerCase().includes(term)); });
    });
</script>
</body>
</html>