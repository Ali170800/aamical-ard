            ...
            <div class="p-6">
                        ...
                        <div>
                            <p class="font-bold text-gray-900">
                                ...
                                <span class="text-blue-600 text-sm font-semibold uppercase tracking-wider">
                                    <%= request.getAttribute("role_auteur_" + p.getId()) %>
                                </span>
                            </p>
                            <p class="text-xs text-gray-400">
                                <%= request.getAttribute("date_publication_" + p.getId()) %>
                            </p>
                        </div>
                    </div>
                ...
                <div class="flex gap-6 border-t pt-4">
                    ...
                    <button onclick="document.getElementById('com-<%= p.getId() %>').classList.toggle('hidden')"
                        ...
                        Commentaires (<span id="com-size-<%= p.getId() %>"><%= (p.getCommentaires() != null) ? p.getCommentaires().size() : 0 %></span>)
                    </button>
                </div>
                ...
                <div id="com-<%= p.getId() %>" class="hidden mt-6 bg-gray-50 p-4 rounded-2xl">
                    ...
                    <form onsubmit="posterCommentaire(event, '<%= p.getId() %>')" class="mb-4">
                        ...
                        <textarea name="commentaire" required placeholder="Écrire un commentaire..."
                                  class="w-full p-2 border rounded-xl" rows="2"></textarea>
                        <button type="submit"
...
// ✅ NOUVELLE VERSION AJOUTÉE SANS CASSER LE RESTE
async function posterCommentaire(e, id) {
    e.preventDefault();
    const form = e.target;

    const pubId = form.querySelector('input[name="publicationId"]').value;
    const texte = form.querySelector('textarea[name="commentaire"]').value;

    const params = new URLSearchParams();
    params.append('publicationId', pubId);
    params.append('commentaire', texte);
    ...
    try {
        ...
        if (res.ok) {
            ...
            const counter = document.getElementById('com-size-' + id);
