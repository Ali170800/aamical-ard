...
<%@ page import="com.amical.ard.entites.*, com.amical.ard.dao.*, com.amical.ard.utils.EntityManagerHelper, jakarta.persistence.EntityManager, java.time.LocalDateTime, java.time.Duration" %>
<%
    Election election = (Election) request.getAttribute("election");
    String eId = request.getParameter("electionId");
    long totalVotes = 0;
    String tempsRestant = "Calcul en cours...";
    ...
    if (election == null && eId != null && !eId.isEmpty()) {
        ...
        if (election != null && election.getCandidats() != null) {
            ...
            estTermine = maintenant.isAfter(election.getDateFin());
            if (!estTermine) {
                Duration duree = Duration.between(maintenant, election.getDateFin());
                tempsRestant = duree.toDays() + "j " + duree.toHoursPart() + "h " + duree.toMinutesPart() + "min";
            } else {
