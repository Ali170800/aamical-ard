...
<div class="bg-gradient-to-r from-indigo-600 via-blue-600 to-indigo-700 text-white p-6 sm:p-8 rounded-3xl shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
    ...
    <div>
        ...
        <span class="inline-block bg-white/20 text-indigo-100 text-xs font-semibold uppercase px-3 py-1 rounded-full mb-3">
            Espace Étudiant
        </span>
        ...
        <p class="text-indigo-100 text-sm mt-3 flex flex-wrap items-center gap-2">
            ...
            Niveau :
            <span class="font-bold text-white">${etudiantConnecte.niveau}</span>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div>
            ...
            <p class="text-slate-500 text-sm mt-2">
                Découvrez les activités et publications.
            </p>

        </div>

        <a href="${pageContext.request.contextPath}/liste-publications"
           class="bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold w-full md:w-auto text-center">
...
<div class="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div class="p-4 bg-slate-50 rounded-2xl">
            <p class="text-xs text-slate-400">Identifiant</p>
            <p class="font-bold text-indigo-700">#${etudiantConnecte.id}</p>
        </div>

        <div class="p-4 bg-slate-50 rounded-2xl">
            <p class="text-xs text-slate-400">Nom</p>
            <p class="font-bold">${etudiantConnecte.nom}</p>
        </div>

        <div class="p-4 bg-slate-50 rounded-2xl">
            <p class="text-xs text-slate-400">Prénom</p>
            <p class="font-bold">${etudiantConnecte.prenom}</p>
        </div>

        <div class="p-4 bg-slate-50 rounded-2xl">
            <p class="text-xs text-slate-400">Filière</p>
            <p class="font-bold text-indigo-700">${etudiantConnecte.filiere}</p>
        </div>

        <div class="p-4 bg-slate-50 rounded-2xl">
            <p class="text-xs text-slate-400">Niveau</p>
            <p class="font-bold">${etudiantConnecte.niveau}</p>
        </div>
    ...
    <c:if test="${not empty etudiantLogement and not empty etudiantLogement.appartement}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="p-5 bg-indigo-50 rounded-2xl">
                <p class="text-xs text-indigo-600">Appartement</p>
...
<div class="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border text-center">
    ...
    <a href="${pageContext.request.contextPath}/etudiant/elections"
       class="mt-4 inline-block bg-violet-600 text-white px-6 py-3 rounded-2xl font-bold w-full md:w-auto">
...
<div class="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border text-center">
    ...
    <p class="text-slate-500 text-sm mt-2">
        Consultez les caravanes disponibles.
    </p>

    <a href="${pageContext.request.contextPath}/etudiant/caravanes"
       class="mt-4 inline-block bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold w-full md:w-auto">
