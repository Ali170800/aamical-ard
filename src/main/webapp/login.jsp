    ...
    <div class="p-6 md:p-8">
        ...
        <div id="loginForm">
            <form action="${pageContext.request.contextPath}/login" method="post">
                <div class="mb-5">
                    <label class="block text-slate-600 mb-2 font-medium">Email ou Login</label>
                    <input type="text" name="login" required class="w-full px-4 py-3 border border-slate-300 rounded-2xl">
                </div>
                <div class="mb-4">
        ...
        <div id="activationForm" class="hidden">
            <form id="step1Form" action="${pageContext.request.contextPath}/admin/verifier" method="post">
                <div class="mb-5">
                    <label class="block text-slate-600 mb-2 font-medium">Email</label>
                    <input type="email" name="email" required class="w-full px-4 py-3 border border-slate-300 rounded-2xl">
                </div>
                <div class="mb-6">
                    <label class="block text-slate-600 mb-2 font-medium">Rôle</label>
                    <select name="role" required class="w-full px-4 py-3 border border-slate-300 rounded-2xl">
                ...
                <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-2xl">Vérifier mon compte</button>
            </form>

            <form id="step2Form" action="${pageContext.request.contextPath}/admin/activer" method="post" class="hidden">
                <input type="hidden" name="email" id="hiddenEmail">
                <div class="mb-5">
                    <label class="block text-slate-600 mb-2 font-medium">Créer un mot de passe</label>
                    <input type="password" name="motDePasse" required class="w-full px-4 py-3 border border-slate-300 rounded-2xl">
                </div>
                <div class="mb-6">
                    <label class="block text-slate-600 mb-2 font-medium">Confirmer le mot de passe</label>
                    <input type="password" name="confirmMotDePasse" required class="w-full px-4 py-3 border border-slate-300 rounded-2xl">
                </div>
                <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-2xl">Activer mon compte</button>
            </form>
        </div>
    </div>
</div>
...
<script>
    ...
    window.onload = function () {
        ...
        document.getElementById("hiddenEmail").value = "<%= request.getAttribute("email") %>";
        <% } %>
    }
</script>
